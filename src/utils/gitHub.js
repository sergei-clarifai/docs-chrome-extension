import { loadConfigFromLocal } from './localStorage';

const { username: githubUsername, token: githubToken } = loadFromLocal('github');
...
export async function fetchFolderFiles(userName, repoName, path, ref) {
  ...
  for (let i = 0; i < reponse.length; i++) {
    ...
    if (item.type === 'dir') {
      const subResult = await fetchFolderFiles(userName, repoName, item.path, ref);
...
export async function fetchFolderFiles(userName, repoName, path, ref, fetchContent = true) {
  const giturl = `https://api.github.com/repos/${userName}/${repoName}/contents/${path}${ref ? `?ref=${ref}` : ''}`;
  ...
  for (let i = 0; i < reponse.length; i++) {
    ...
    if (item.type === 'dir') {
      const subResult = await fetchFolderFiles(userName, repoName, item.path, ref, fetchContent);
      result = result.concat(subResult);
    } else {
      result.push(item);
      if (fetchContent) {
...
export async function getDocsFromGithub() {
  ...
  if (!cachedFileList || !cachedFileList.length) {
    const filenames = await fetchFolderFiles(userName, repoName, path, branchName, false);
  ...
  const docsFileNames = JSON.parse(cachedFileList);

  const selectedDocFiles = Array(4).fill(0).map(() => {
